...
function masterFunc(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        let char = inputsArr[i].split(' ')[1][0]
        let pw = inputsArr[i].split(' ')[2]
        let [smallerNum, biggerNum] = inputsArr[i].split(' ')[0].split('-')

        if (pw[smallerNum - 1] === char && pw[biggerNum - 1] !== char) {
            numOfValid++
        } else if (pw[smallerNum - 1] !== char && pw[biggerNum - 1] === char) {
            numOfValid++
        } else { // It's always good to "close it out" with an 'else', otherwise you might think you've caught all scenerios when you in fact haven't.
            // console.log('invalid') // this is only commented out to minimize what's logged
        }
    }
...
console.log(masterFunc(inputsArr)) // 509 // correct